...
def cls_model_embedding(df: DataFrame) -> DataFrame:
    ...
    embedding = cls_model.encode(
        utterance, convert_to_tensor=True).cpu()  # type: ignore
...
for f_name in tqdm(os.listdir(dataset_file_path), total=len(os.listdir(dataset_file_path))):
    # 都議会だより
    digest_df = pd.read_csv(
        f"{dataset_file_path}/{f_name}/digest.csv", index_col=0)
    # 都議会会議録
    assembly_df = pd.read_csv(
        f"{dataset_file_path}/{f_name}/assembly.csv", index_col=0)
    # cls embedding
    ...
    embedded_digest_df.to_pickle(
        f"{dataset_file_path}/{f_name}/embedded_digest.pkl")
    embedded_assembly_df.to_pickle(
        f"{dataset_file_path}/{f_name}/embedded_assembly.pkl")
# %%
